...
def find_nearest_ngo(victim, ngos, crisis_locations):
    ...
    for crisis_location in crisis_locations:
        ...
        if victim["priority"] >= crisis_location["crisis_level"]:
            ...
            for ngo in ngos:
                # Only consider NGOs with capacity > 0
                if ngo["capacity"] > 0:
    ...
    # Check if the NGO is available and has enough capacity
    if available_ngo and available_ngo["capacity"] > 0:
        # Reduce the NGO's capacity (simulating allocation)
        available_ngo["capacity"] -= 1
        return available_ngo, min_distance, max_crisis_level
    else:
...
for victim in victims:
    ...
    if nearest_ngo:
        ...
        print(f"NGO Capacity: {nearest_ngo['capacity']} (after allocation)")
        print("---------")
    else:
